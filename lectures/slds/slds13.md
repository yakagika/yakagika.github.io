...
::: warn
基本的に本章では, 機械学習用ライブラリ`scikit-learn`(`uv add`や`import`時には省略形の`sklearn`)を利用します. 以下のいくつかのライブラリも含めて `uv add`しておいてください.
...
![決定木のイメージ](/images/slds/ch13/decision-tree-analysis-image.png)
...
~~~ sh
uv add pydotplus ipython scikit-learn
~~~
