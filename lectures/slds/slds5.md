...
![データ分析の流れ](/images/slds/ch5/data-science-flow.png)
...
ライブラリを利用するためには,まずライブラリを皆さんのuvプロジェクトににインストールする必要があります.
特定のプロジェクト内で
`uv add [ライブラリ名]`
と入力することでライブラリを追加することが可能です.

今回は `pandas`をインストールするので`slds`フォルダで`uv init -p 3.11.9 pandas-test`したうえで, `uv add pandas`と入力しましょう. 文字が沢山表示されますが, 問題ありません. 画面が止まるまで待ちましょう.
なお, ライブラリを削除する場合は
`uv remove [ライブラリ名]`
となります.
...
![](/images/slds/ch5/sublime-utf-8-bom.png)
...
![](/images/slds/ch5/excel-procedure1.png)
...
:::
![](/images/slds/ch5/excel-procedure2.png)
