...
![データの作られ方](/images/slds/ch6/statistical-data-flow.png)
...
**(統計理論の判断問題のため, 講義中の議論 / 別途解説に委ねる. 以下は判定の指針)**

判定の基本軸:

- **名義尺度**: ラベルの分類のみ. 順序・等間隔・絶対 0 のいずれも持たない.
- **順序尺度**: 順序はあるが間隔が等しいとは限らない.
- **間隔尺度**: 順序 + 等間隔. 絶対 0 は持たない.
- **比例 (比) 尺度**: 順序 + 等間隔 + 絶対 0 (例: 長さ, 重さ, 個数).

参考: 「`0` が意味を持つか (絶対 0 か)」「差や比が意味を持つか」を順に問えば 4 尺度の判別がつく.
...
![e-stat screenshot1](/images/slds/ch6/e-stat-screenshot1.png)
![e-stat screenshot2](/images/slds/ch6/e-stat-screenshot2.png)
...
![](/images/slds/ch6/e-stat-salary1.png)
...
![](/images/slds/ch6/e-stat-salary2.png)
...
![神Excel](/images/slds/ch6/kami-excel.png)
...
![ヘッダーの作成](/images/slds/ch6/e-stat-salary5.png)

![データのコピー](/images/slds/ch6/e-stat-salary6.png)
...
![翻訳されたデータ](/images/slds/ch6/e-stat-salary7.png)

![Excelの翻訳機能](/images/slds/ch6/e-stat-salary8.png)
...
![データの保存](/images/slds/ch6/e-stat-salary9.png)
...
![DataFrameのイメージ](/images/slds/ch6/data-frame-image.png)
:::
...
![値の抽出](/images/slds/ch6/pandas-select-practice.png)

<details class="protected" data-pass="yakagika">
    <summary> 回答例 </summary>

- ① `Manufacturing` (index 1) ~ `AFFC` (index 13) の全列 → スライスで範囲指定 (`loc` のスライスは末尾を含む).
- ② `Finance` (index 4) の `Sal` 列という単独セル → `.at`.
- ③ `Transport` (6), `Info` (8), `Med` (10) という連続しない行の `Ind` 列 → 行にリストを渡す.
...
# ① Manufacturing(1)〜AFFC(13) の全列 (loc のスライスは末尾を含む)
print('① ---')
print(df.loc[1:13, 'Ind':'Sal'])

# ② Finance(4) の Sal 列 (単独セルなので .at)
print('② ---')
print(df.at[4, 'Sal'])

# ③ Transport(6), Info(8), Med(10) の Ind 列 (行にリストを渡す)
print('③ ---')
print(df.loc[[6, 8, 10], 'Ind'])
~~~
...
論理演算 (`%` による偶奇判定や `==` による一致判定) で Bool のリストを作り,
それを `df[...]` に渡して `True` の行だけ取り出す → さらに `['列名']` で列を絞る, という 2 段構えで考える.
...
# 3. Ind が 'Info' の行の Sal 列
print('3. ---')
print(df[df['Ind'] == 'Info']['Sal'])
~~~
...
5. 給与列のうち一番高い数値を2倍してください

6. 元のDataFrameとコピーされたデータフレームの中身とidを確認してください.

<details class="protected" data-pass="yakagika">
    <summary> 回答例 </summary>

「どこまでをサービス業 (第三次産業) とみなすか」は解釈に幅があるので,
ここでは代表的な業種を `isin` で選んでいる. 抽出後に `.copy()` しているため,
最後の `id` 確認で元の `df` とコピー先が **別オブジェクト** であること,
元の `df` が変更されていないことが確認できる.
...
# 2. サービス業だと思われる行を抽出して .copy()
#    (どこまでをサービス業とみなすかは解釈による)
service_inds = ['Wholesale', 'Accomodation', 'Finance',
                'Real Estate', 'Info', 'Edu', 'Med', 'Suervice']
service = df[df['産業'].isin(service_inds)].copy()

# 3. 0 から始まる index に振り直す
service.reset_index(inplace=True, drop=True)

# 4. 産業列を日本語訳
trans = {'Wholesale': '卸売', 'Accomodation': '宿泊',
         'Finance': '金融', 'Real Estate': '不動産',
         'Info': '情報通信', 'Edu': '教育', 'Med': '医療',
         'Suervice': 'サービス'}
service['産業'] = service['産業'].map(trans)

# 5. 給与列の最大値を 2 倍にする
imax = service['給与'].idxmax()   # 最大値の index
service.at[imax, '給与'] = service.at[imax, '給与'] * 2

# 6. 中身と id を確認 (.copy() したので元 df は不変, id も異なる)
print('元 df ---\n', df)
print('service ---\n', service)
print('id df     :', id(df))
print('id service:', id(service))
~~~
...
`numpy`をプログラムで利用しましょう. `uv add` で追加したパッケージは `uv run` 実行時に自動で仮想環境に同期されるため,パスの設定などを気にする必要はありません.
...
列名を `key`, 列の値のリストを `value` とした辞書を `pd.DataFrame()` に渡すのが簡単.
作成直後は `age` が `int64` なので, `.astype('float64')` で変換して元の列に代入する.

~~~ python
import pandas as pd

# 辞書で DataFrame を作成
df = pd.DataFrame({'name': ['Taro', 'Jiro', 'Hanako'],
                   'age': [20, 30, 40]})
...
![salary_nan.csv](/images/slds/ch6/salary-nan.png)
...
- `index`が偶数の行のみを残して,すべての列を`int`型に変更した後`BOM付きのUTF-8`の`csv`で保存してください.
...
# 1. データの作成と編集
# ============================================
# columns が '1'..'5', index が 0..19,
# '1' 列は index*1, '2' 列は index*2, ... '5' 列は index*5
df = pd.DataFrame({
    str(i): [j * i for j in range(20)] for i in range(1, 6)
})

# '3' 列を 2 倍した '6' 列を追加
df["6"] = df["3"] * 2

# index = 20 の行を追加 (各列も index*列番号 のルールで埋める)
df.loc[20] = [20 * i for i in [1, 2, 3, 4, 5, 6]]

# 各行の平均値の 'mean' 列を追加
df["mean"] = df.mean(axis=1)

# index 偶数のみを残す → int に変換 → BOM 付き UTF-8 で CSV 保存
df_even = df[df.index % 2 == 0].astype(int)
df_even.to_csv("output.csv", encoding="utf-8-sig")
...
# 2. 外部データの読み込みと編集
# ============================================
# 神戸大学 RIEB 近世経済データ (米相場) Excel を取得し,
# 米相場列に欠損値が無いように整え, 基本構造を確認し, 平均値を求める.
# 注: 実際には Excel の URL を pd.read_excel に直接渡せない場合があるので,
#     ダウンロードして手元に保存してから読み込むのが安全.

# 例 (近世経済データの場合, シート名・列名はファイルにより異なる):
# df_rice = pd.read_excel("kinsei.xlsx", sheet_name="米相場")
# df_rice = df_rice.dropna(subset=["米相場"])
# print(df_rice.info())
# print(df_rice.describe())
# print("平均:", df_rice["米相場"].mean())
~~~

外部 Excel の取り扱いはファイルごとの構造に依存するため, 実データを開いてから列名・シート名を合わせて調整してください.
