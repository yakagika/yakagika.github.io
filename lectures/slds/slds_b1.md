...
残高が下がったときに自動でクレジットを買い足す設定 (auto-recharge) を無効にしておけば, **前払いした金額がそのまま支出の上限になります.** プログラムの誤りでループが止まらなくなっても, 残高を使い切った時点でリクエストが失敗するだけで, 請求が青天井に膨らむことはありません. 少額 (10〜25ドル程度) をチャージして使うのが安全です.
...
**1. 開発者アカウントを登録する.** [Developer Console](https://console.x.com)からサインアップし, 開発者規約に同意します. 利用目的を尋ねられた場合は, **250文字以上の英文で**回答します. 研究・教育目的であれば, その内容をそのまま書けば通ります.

![利用目的の記入欄](/images/slds/ch15/x-reason.png)

**2. クレジットをチャージする.** 従量課金は前払い制なので, Developer Consoleの課金画面からクレジットを購入します. 前節のとおりチャージ額が支出の上限になるため, まずは10〜25ドル程度から始めます. このときauto-rechargeは無効のままにします. 有効にすると残高が下がるたびに自動で買い足され, チャージ額が上限として働かなくなります. 請求サイクルごとの上限額 (spending limit) も併せて設定しておくと, 二重の安全装置になります.

**3. アプリを作成してトークンを発行する.** Developer Consoleの`New App`でアプリを作成すると`Bearer Token`が表示されます. クリップボードにコピーして安全な場所に保存しましょう. このトークンは一度しか表示されません. 忘れた場合は別のトークンを再生成する必要があります.

::: note
旧デベロッパーポータルの`Standalone Apps`にあるアプリ, つまりProjectに属していないアプリのトークンは従量課金では使えず, リクエストは`403 client-not-enrolled`で拒否されます. 同じアプリでトークンを再生成しても解決しないので, Developer Consoleでアプリを新規に作成し, そのトークンを使ってください.
:::
...
::: note
エラーの本文にはどの理由で失敗したかが書かれています. 残高が尽きた場合は`403`, 認証トークンが誤っている場合は`401`, 短時間にリクエストを送りすぎた場合は`429`が返るので, 失敗したらまず本文を読んでください. `403`はアプリがProjectに属していないときにも返り, その場合は本文に`client-not-enrolled`と書かれています.
:::
