...
**場合分けで考える ― 複雑な関数を「入力の形」ごとに分ける**

パターンマッチは単なる構文ではなく **考え方の道具** でもあります. 複雑そうな関数も, 「引数がどんな形のときに何を返すか」を **場合に分けて** 並べれば, 一つ一つは単純な式になります.

頭の中でやっていること:

1. 引数が取りうる **形** を数え上げる (リストなら「空 `[]`」「先頭+残り `x:xs`」など).
2. **いちばん単純な場合** から先に書く (多くは空・ゼロ・1 要素).
3. 残りの, より複雑な場合を順に埋める.

`strHead` では「空 `[]`」「1 要素 `[x]`」「それ以外 `x:_`」の 3 つに分けました. **単純な場合から並べて抜けがないか確かめる** この習慣は, この後のガードや case 式, そして再帰関数を組み立てるときの土台になります.
...
**具体例で手を動かす ― 困ったら小さな値を入れて手で評価する**

`total (x:xs) = x + total xs` を見て「何をしているか分からない」と感じたら, 私はいつも **いちばん小さい具体的な入力を 1 つ選んで, 紙の上で一段ずつ書き換えて** みます. 上の `total [1,2,3]` の展開がまさにそれです.

頭の中でやっていること:

1. 小さな具体例を選ぶ (要素 2〜3 個の `[1,2,3]` など).
2. 定義の左辺と一致するパターンを探し, 右辺で **置き換える**.
3. もう書き換えられなくなるまで 1 行ずつ繰り返す.

抽象的なまま手が止まったら, 具体例に **降りて** 計算し, 戻って一般化する. この往復が, `xs` や `total xs` を「正体不明の何か」ではなく「具体的に動かせるモノ」として掴む近道になります.
...
`total`では,終了状態として, **空のリストが引数となったとき**を先に作ります. 終了状態の右辺は, 決まった値を取るようにして関数を再帰させないようにしましょう.
...
コツとしては,関数の状態としてできるだけ単純な状況を最初に考えて取り敢えず実装して見るようにしましょう. 
**空のリスト`[]`** を受け取ったら0となる,それ以外のパターンではどのようになるかを次に考えてみます.

一つだけ要素を持つリスト `[x]`の場合は, その和は `x` となるので,
...
複数要素を持つリスト `x:xs`の場合は, `x` と ｢`xs`の和｣ の和 となるので

~~~ total
total (x:xs) = x + ｢`xs`の和｣
~~~

となります. このとき`xs`は`[Int]`なので **`[Int]` を受け取ってその要素の和(`Int`)を返す**処理はどのようなものかを考えてみます.
すると, 今作成している `total :: [Int] -> Int` がまさに **`[Int]` を受け取ってその要素の和(`Int`)を返す**関数であることが分かります.
そこで, ｢`xs`の和｣ は `total xs` と書けることが分かります.

~~~ haskell
total (x:xs) = x + total xs
~~~

このように, 再帰関数を作成する上で難しい考え方として **まだ完成していないが今作っている関数が既に機能していると想定して利用すること**が挙げられます.
今 **`[Int]` を受け取ってその要素の和(`Int`)を返す関数** を作る中で,その関数が既に完成しているとみなして利用することで, 再帰関数が完成します.

このように再帰関数を作成する場合には,

1. まず,欲しい関数の型を明確にする

2. できるだけ単純なパターンがどの様になるかを考える

3. 終了状態のパターンを実装する

4. 複雑なパターンを順次考えていく, その中でその関数自体が既に完成しているとみなして,利用する

といった考え方をすると,上手く作成できる場合が多いです.
また, これは静的型付け言語一般のコツですが, それぞれのパターンや,計算の中で **この部分のデータ型は何か** を常に意識することも非常に重要です.

:::

`total` では終了状態が ｢空リスト `[]`｣ の1つだけでしたが, 終了状態は複数あっても構いません. 
本章で何度も登場している `fib` も再帰関数の一例です.
...
`total` は自分を 1 回, `fib` は 2 回呼び出しました. もう少し実用的な例として, リストを昇順に並べ替える **挿入ソート** を再帰で書いてみます. 考え方は上の「再帰関数を実装するコツ」でみた **完成前提で自分を使う** そのものです.

- **終了状態**: 空リストはすでに整列済みなので, そのまま `[]` を返す.
- **それ以外** `(x:xs)`: 「先頭 `x`」と「残り `xs`」に分ける. **`xs` を並べ替える処理は, いままさに作っている `insertionSort` 自身**だとみなすと, `insertionSort xs` で整列済みの列が得られる. あとはその整列済みの列へ `x` を正しい位置に挿し込めば完成.
...
-- 整列済みリストへ 1 要素を正しい位置に挿し込む (これも再帰)
insert :: Int -> [Int] -> [Int]
insert x []     = [x]
insert x (y:ys)
  | x <= y    = x : y : ys        -- x が y 以下ならここが居場所
  | otherwise = y : insert x ys   -- まだ大きいので 1 つ先へ送る

main = print $ insertionSort [3,1,2]  -- [1,2,3]
~~~

`insertionSort` は「残りを並べ替えてから先頭を挿す」, `insert` は「居場所が見つかるまで 1 つずつ先へ送る」という, **2 つの再帰が組み合わさった** 関数です. `insertionSort [3,1,2]` の流れを追うと, まず `insertionSort` が末尾までほどけ, 続いて `insert` が正しい位置を探していきます.

~~~
insertionSort [3,1,2]
= insert 3 (insertionSort [1,2])
= insert 3 (insert 1 (insertionSort [2]))
= insert 3 (insert 1 (insert 2 (insertionSort [])))
...
このように, 再帰は単なる繰り返しの言い換えにとどまらず, ソートのような実際のアルゴリズムを素直に書き下せます.

::: note

**このソートはどれくらい速い?** 挿入ソートは入力の並びによって手間が変わり, 逆順の入力では要素数 $n$ のほぼ 2 乗に比例した回数の比較が必要になります. こうした「アルゴリズムがどれだけ速いか (計算量)」の測り方と, マージソート・クイックソートを含めた比較は [補足A](fp_a1.html) で扱います (そこで出てくる `foldr insert []` は, 上の `insertionSort` を高階関数で書き直したもので, 同じ関数です).
...
2. 与えられた整数のリストを引数にとり,要素を文字列に変換する `toStr :: [Int] -> [Strings]`関数を新しく実装して下さい.

文字列への変換は `show` を使いましょう.

~~~ haskell
toStr [1,2,3] -- ["1","2","3"]
~~~

3. 与えられた整数のリストを引数にとり,要素毎にFizzBuzzを実行した結果を文字列のリストで返す関数
