...
本章は 4 部構成です. **圏** では, 圏を定義し, Haskell の型と関数がそのまま圏をなすことを確かめます. **関手** では, 圏の準同型 = 関手を手作りの実例から一般定義へ進め, Haskell の型クラス `Functor` と関手則に接続します. **多相データ型** (関手のモデルたち) では, 関手のモデル (実例) として多相データ型を一つずつ増やします. 失敗を表す `Maybe`, 二者択一の `Either` ([第7章](fp7.html)で「後の章で扱う」と先送りした型), 既習のリスト, 実務で頻出の辞書 `Map`, その内部構造を確かめるツリーです. それぞれが **なぜ実用で必要になるか** も, ここで一つずつ語ります. **自然変換** で, 上の対応表を完成させます.
...
2. [関手](#関手)と[型クラス Functor](#リスト関手と型クラス-functor): 射の対応と Haskell の `Functor`.
...
[第4章](fp4.html)以来, 私たちは `g . f` と書けば関数がつながり, `id` を挟んでも何も変わらないことを当たり前に使ってきました. プログラムを「小さな関数の合成」として設計するこのやり方を支えているのは, 結合律と恒等律という 2 つの法則です. 圏とは, この「当たり前」を組 + 法則として明文化した構造です.
...
[第8章](fp8.html)ではモノイドの **台の要素 (値)** だった `[1,2]` や `[3]` が, この読み方では 1 本ずつの **射** になります. 「リストを 2 つ選んで連結する」ことは「射を 2 本選んで合成する」ことにほかならず, モノイドの結合律・単位元律はそのまま圏の結合律・恒等律として読めます. この「モノイド = 一点圏」の読み方[^delooping]は, 次節で **最初の関手を作る材料** としてさっそく使います.

[^delooping]: モノイドから対象 1 つの圏を作るこの対応は, 圏論の文献では **delooping** と呼ばれ, モノイド $M$ から作った一点圏を $\mathbf{B}M$ と書きます.
...
[第7章](fp7.html)の再帰的データ型では, 構成子の束 (たとえば `Nat` の 0 項演算 `Zero` と 1 項演算 `Succ`) を生成作用素 $\Phi$ に畳み込みました. 圏論ではこれをさらに進め, アリティのばらばらな演算たちを直和で束ねて **たった 1 本の射** $\alpha : F(A) \to A$ にします (`Nat` なら $\alpha = [\texttt{Zero}, \texttt{Succ}] : () + \mathrm{Nat} \to \mathrm{Nat}$). この $F$ は本節の次に学ぶ **関手** であり, この形の組 $(A, \alpha)$ は **$F$-代数**, その中で最初に出来上がるものは **始代数 (initial algebra)** と呼ばれます. 「再帰的データ型 = 生成作用素の最小不動点」の圏論版です (名前だけ挙げておきます. [第10章](fp10.html)のモナドの単位 $\eta : \mathrm{Id} \Rightarrow T$ も, 実は「単位対象からの射としての 0 項演算」パターンの再演です).
:::
...
[第4章](fp4.html)の `map f` は「リストの中身にだけ `f` を当てる」操作でした. 同じことを `Maybe` の中身にも, ツリーの中身にもしたい. しかも容器ごとに別々の書き方を覚えるのではなく, 「中身への適用」を 1 つの共通の操作として持ちたい. この需要に応える道具が関手です.

[第8章](fp8.html)の準同型の章では, 構造を保つ写像を「集合の対応であって, **構造の成分を対応先の成分に写すもの**」として一般化し, モノイド準同型 (演算と単位元を保つ)・単調写像 (順序を保つ) などを 1 つの表に並べました. 圏も「組 + 法則」で定義された構造ですから, 同じ発想で「圏の構造 (**合成と恒等射**) を保つ写像」が考えられるはずです. それが **関手 (functor)** で, 冒頭の階段の 3 段目にあたります. Haskell に型クラスとして用意されているのは関手のうち特殊なケースだけ ([第8章](fp8.html)でマグマや群に標準クラスが無かったのと同じ事情) なので, 一般の定義や型クラスからではなく, まず **関手そのものを具体的に 2 つ, 手で構成する** ところから入ります.
...
リストのリスト `[[a]]` や `Maybe (Maybe a)` のような「関手を 2 回適用した形」は, [第10章](fp10.html)で, それ自体 1 つの対象として扱いたくなります (モナドの乗法 $\mu : T \circ T \Rightarrow T$ の材料になります). そのためには「関手どうしを合成する」演算が要ります. その演算が住む場所を, ここで作ります.
...
Haskell では, Hask から Hask への関手の「射の対応」を型クラスのメソッドにしています. それが `Functor` クラスで, 射の対応の名前が `fmap` です. 個々のインスタンスでは, 型構築子 `f` が対象の対応を, `fmap` の実装が射の対応を担い, 関手則と合わせて 1 つの関手になります. 注意してほしいのは抽象度の段差です. **`fmap` という名前そのものは個々の関手ではなく, あらゆる関手の「射の対応」を同じ名前で呼べるようにした一般化** であり, 関手より 1 段抽象的な道具です. 先に関手そのもの (手作りの $F$) を持っているからこそ, この一般化を「何の一般化か」を見失わずに読めます.
...
`fmap` は「`a` から `b` への関数」を「`f a` から `f b` への関数」に持ち上げます. これがまさに「射 $f : a \to b$ を $F\,f : f\,a \to f\,b$ に対応させる」操作です. つまり `fmap` は, リスト専用だった `map` を **任意の関手に一般化** したものです. `map` の型 `(a -> b) -> [a] -> [b]` の `[]` を一般の型構築子 `f` で置き換えると, ちょうど `fmap :: (a -> b) -> f a -> f b` になります.
...
3 つの結果はすべて一致します. `fmapList f` は「要素に使う関数 `f`」を「リスト全体に使える関数」へ変換したもので, 変換後の関数は空リストには何もせず, `(x : xs)` の各要素だけを `f` で置き換えます. この実装ではリストの長さと並びも変わりません. ただし, これは `map` の具体的な振る舞いであり, 関手則そのものではありません. 関手であることを保証する条件は, あくまで恒等射と合成を保つ 2 本の等式です.

標準ライブラリでは, この射の対応が次のインスタンス宣言で与えられています (実際の定義は `fmap = map` の 1 行で, 下はそれを場合分けに開いた等価形です).

~~~ haskell
-- リスト: 標準ライブラリの定義を場合分けに開いたもの (等価)
...
::: warn
このインスタンスは標準ライブラリ (`base`) で定義済みです. **同じ宣言を自分のファイルに書くと重複インスタンスのエラーになる**ので, 上のコードは読むための抜粋です. この後の `Maybe`・`Either a` の宣言も同様です (自作の型にインスタンスを書く練習は, このあとの `Box` とツリーで行います).
:::
...
[第8章](fp8.html)の代数則と同じく, `Functor` インスタンスにも守るべき法則があります. **関手則 (functor laws)** といい, 「関手の一般定義」の節の保存則 (恒等射と合成を保つ) を `fmap` の言葉に直したものです.

$$\mathrm{fmap}\ \mathrm{id} = \mathrm{id}, \qquad \mathrm{fmap}\ (g \circ h) = \mathrm{fmap}\ g \circ \mathrm{fmap}\ h$$

`fmap id = id` は「各中身に何もしなければ全体も変わらない」, `fmap (g . h) = fmap g . fmap h` は「持ち上げてから合成しても, 合成してから持ち上げても同じ」という要請です. リスト関手では, `map` について検査した 2 つの等式がちょうどこの形でした. この後の節で `Maybe` やツリーのインスタンスを与えるたびに, この 2 本が成り立つことを確認していきます.

::: warn
`Functor` クラスも, `Semigroup` などと同様に **関手則をコンパイラは検査しません**. たとえば `fmap id` で木の形まで変えてしまうような実装もコンパイルは通りますが, 恒等射の保存則を破るため関手ではありません. 法則を守るのはプログラマの責任です ([第8章](fp8.html)のコラムで触れた QuickCheck で, `fmap id == id` などを性質として確認できます).
:::
...
と, どちらの順でも `Just 11` で一致します. **全域的でパラメトリックな実装**に限れば, 関数は中身の値を型ごとに調べられないため, この自然性は型から導かれます. ただし実際の Haskell では, `seq`・`undefined`・`unsafeCoerce` などがこの前提を破り得ます. 本章では, 先の Hask の節と同じく, それらを除いた理想化のもとで「型から自然性が従う」と読みます.
...
`maybeToList` のような一様変換は, 部品として合成したくなります (変換した結果をさらに別の一様変換へ渡す). 変換どうしの合成が圏の公理を満たすなら, 変換の等式変形 ([第10章](fp10.html)のモナド則がその実例です) を圏の言葉で安全に行えます.
...
自然性も引き継がれます. 前節の可換四角形を 2 つ横に継げば, 外周がまた可換な四角形になります. Haskell 側でも, 全域性とパラメトリシティの前提を満たす関数どうしなら, 合成後も同じ前提を満たすため自然性が成り立ちます.
