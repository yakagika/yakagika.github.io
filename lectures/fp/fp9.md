...
本章は 2 部構成です. 前半 **多相データ型** では, 型引数 `a` を持つデータ型を整理します. [第8章](fp8.html)から移ってきたリスト・ツリーに加え, 実用的な辞書型 `Map`, 失敗を型で表す `Maybe`, 二者択一を表す `Either` を扱います. これらは [第7章](fp7.html)で「後の章で扱う」と先送りした型でもあります.
...
以下では, 型引数 `a` を持つ多相データ型を 5 つ見ていきます. まず **リスト `[a]` / ツリー `Tree a`** ([第8章](fp8.html) から移してきた **モノイド** の実例) と, 実用的な辞書型 **`Map`** を扱います. これらは「型引数を持つ多相データ型」であると同時に, モノイドや関手といった構造も備えます (関手は後半 [圏論的解釈](#圏論的解釈) の **Functor** の節で扱います). その後, 失敗を表す **`Maybe`**, 二者択一を表す **`Either`** に進みます.
