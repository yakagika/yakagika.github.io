...
[第8章](fp8.html) のモノイドの note で見た「組 + 法則」の形は, 圏にもそのまま当てはまります: 圏とは「**組 (射の集まり; $\mathrm{dom}$, $\mathrm{cod}$, $\circ$) であって, 結合律・恒等律という法則を満たすもの**」です. モノイドと見比べると, 3 つの読み替えが起きています.

- **台に当たるのは射の集まり**です (対象ではありません). モノイドの台が「値」だったのに対し, Hask の台は「関数」 — [第7章](fp7.html) の集合のあいだの**写像たち**が, 今度は台になります.
- 演算をアリティで数えると, $\mathrm{dom}$ / $\mathrm{cod}$ (射にその入口・出口の対象を対応させる操作) が **1 項演算**, 合成 $\circ$ が **2 項演算** です. ただし $\circ$ の定義域はすべての対ではなく, [第7章](fp7.html) の内包表記で書ける部分 $\{(g, f) \mid \mathrm{dom}\ g = \mathrm{cod}\ f\}$ (型の合う対) に絞られます. 対象が 1 つしかないモノイドではこの条件が常に成り立つため, 制限が見えなかったわけです.
- **恒等射は $\circ$ から一意に定まります** — `mempty` が「あれば $\bullet$ から一意」だったのと同じ理屈です (同じ対象の恒等射 $u$, $u'$ について $u = u \circ u' = u'$).

つまり圏は, モノイドのレシピの **台を「値」から「射 (写像)」へ持ち上げ, 演算の定義域を型で絞ったもの** です. この「台の持ち上げ」は [第10章](fp10.html) でもう一段続き (台 = 自然変換), その先にモナドが立ちます.
:::
