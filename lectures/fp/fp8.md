...
[第7章](fp7.html)では「型 = 集合」と捉えました. その集合の上に「演算」を載せ, さらに「どんな法則を満たすか」で構造を分類したものが代数構造です. ここでは, 演算が満たす法則の **少ない順** に, マグマ → 半群 → モノイド → 群 と積み上げていきます.

| 構造 | 演算 | 満たす法則 | 対応する型クラス |
| --- | --- | --- | --- |
| マグマ | 二項演算 1 つ | 閉じている (だけ) | (標準には無い) |
| 半群 | 二項演算 1 つ | 結合律 | `Semigroup` |
| モノイド | 二項演算 + 単位元 | 結合律 + 単位元律 | `Monoid` |
| 群 | 二項演算 + 単位元 + 逆元 | 結合律 + 単位元律 + 逆元律 | (標準には無い) |

## マグマ

最も法則の少ない代数構造が **マグマ (magma)** です. 台集合 $S$ と二項演算 $\bullet : S \times S \to S$ の組で, 要求されるのは **演算が集合の中で閉じている** (結果がまた $S$ の要素になる) ことだけです. 結合律も単位元も要求しません.

Haskell の標準ライブラリにマグマのクラスはありませんが, この二項演算 $\bullet$ に対応する演算子 `(|*|)` をメソッドに持つクラスを学習用に定義してみましょう.
...
身近なマグマの例が **じゃんけん** です. 「2 手を出して勝った方を返す」演算 $a \bullet b$ を考えます (あいこなら左を返すことにします). たとえば $\text{グー} \bullet \text{チョキ} = \text{グー}$ です.
...
マグマに **結合律 (associativity)** を加えたものが **半群 (semigroup)** です. 結合律とは, 二項演算 $\bullet$ が **すべての** $x, y, z$ について
...
例として, 「2 つの整数の **大きい方** を返す」演算 $a \sqcup b = \max(a, b)$ を考えます. `max` は結合律 $(a \sqcup b) \sqcup c = a \sqcup (b \sqcup c)$ を満たす (どこから比べても最大は同じ) ので半群になります. まず数式の $\sqcup$ に対応する 2 項演算子 `(|+|)` を定義し, それを使って `Semigroup` の `(<>)` を与えます.
...
半群に **単位元 (identity element)** を加えたものが **モノイド (monoid)** です. 単位元とは, 「すべての $x$ と演算しても相手を変えない要素 $e$ が **存在する**」という条件で, [第7章](fp7.html) の **存在命題** を使って
...
整数は, **同じ台集合の上に 2 つのモノイド構造** を持ちます. 足し算 (`+`, 単位元 $0$) と掛け算 (`*`, 単位元 $1$) です. Haskell では 1 つの型に `Monoid` インスタンスは 1 つしか付けられないため, `newtype` で包んで区別します.
...
この `Bool2` の上で, 2 つのモノイドを考えます.

- $\wedge$ (`(.&.)`): 単位元は `T` ($\text{T} \wedge x = x$).
- $\vee$ (`(.|.)`): 単位元は `F` ($\text{F} \vee x = x$).

整数のときと同様, 1 つの型に `Monoid` は 1 つしか付けられないので, `newtype` で 2 通りに包みます (組込の `Bool` に対する同じ構造が, `Data.Monoid` に `All` / `Any` として実在します).
...
モノイドに **逆元 (inverse element)** を加えたものが **群 (group)** です. 逆元とは, 「**各要素** $x$ に対して, 演算で打ち消して単位元に戻せる相手 $y$ が **存在する**」という条件で, 全称命題と存在命題を組み合わせて
...
Haskell の標準ライブラリに群のクラスはないので, モノイドを拡張する形で学習用に定義します.
...
有限の群の例として, 「3 で割った余り」の世界 $\mathbb{Z}/3\mathbb{Z} = \{0, 1, 2\}$ における加法を考えます. これは[第7章](fp7.html)で学んだ **列挙型** で表せます.
...
::: note
ここまでの マグマ → 半群 → モノイド → 群 は, 法則を 1 つずつ足して構造を強くしていく階層になっています.

- **マグマ**: 閉じた演算だけ (じゃんけん)
- **半群**: + 結合律 (`Max`)
- **モノイド**: + 単位元 (`Add` / `Mul`, `All2` / `Any2`)
- **群**: + 逆元 ($\mathbb{Z}$ の加法, `Z3`)
