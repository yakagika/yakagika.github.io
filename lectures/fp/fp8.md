...
ここまで見た「代数構造 → 生まれる能力 → Haskell の例」を 1 枚に整理します. 下に行くほど法則・材料が増え, その分だけ **できること (能力)** が増えていく, という読み方をしてください.

| 代数構造 | 足す法則・材料 | 生まれる能力 | 本章の例 |
| --- | --- | --- | --- |
| 半群 $(S, \bullet)$ | 結合律 | 分割統治・並列 (順序は保つ) | `Max`, リスト連結 |
| モノイド $(S, \bullet, e)$ | + 単位元 | **畳み込み** (空も畳める, `foldMap`) | `Add`, `Stats`, `All2` / `Any2` |
| 可換モノイド | + 可換律 | **順序不同で畳める** (分散集計・並べ替え) | `Add`, `Moments` |
| 群 $(S, \bullet, e, {}^{-1})$ | + 逆元 | **取り消し・差分** (引き算で打ち消す) | `Z3`, $\mathbb{Z}$ の加法 |
| 半環 $(S, \oplus, \otimes)$ | 2 演算 + 分配律 | **加重集計・線形結合** | `WMean`, ブール代数 |

同じ「畳み込み」でも, 課す法則によって **どこまで自由に畳んでよいか** が変わります — 結合律だけなら順番を保ったまま分割, 可換律まであれば順不同で分散, 逆元まであれば後から取り消し, 2 演算あれば重み付き. 「自分の型にどの構造を与えるか」は, 裏返せば「その型でどんな集計を許したいか」を選ぶこと, と言えます.
