...
こちらのパーセプトロンは最初の$x_1,x_2$からなる0層と入力を受け取る$y$の2層から構成されていますが,ニューロンの数や,層の数を増やして様々な複雑な条件を表現することが可能になります. しかし,パーセプトロンはそれぞれの重みを人間が設定する必要があり,複雑な条件を満たすネットワークの重みを人間が設定するのは現実的ではありません.そこで,**それぞれの重みを機械学習によってデータから自動で学ぶようにしたものがニューラルネットワークです**.

基本的なニューラルネットワークではネットワークを入力層,隠れ層,出力層の3層に分類します.

![](/images/ch14-NN1.png)

ここで各ニューロンの発火条件は以下のように表現されます.

$$
\begin{align*}
&y = f(w_1x_1 + w_2x_2 + \cdots + w_nx_n + b) \\
&x_1, x_2, \dots, x_n：入力値 \\
&w_1, w_2, \dots, w_n：重み \\
&b：バイアス \\
&f：活性化関数 \\
\end{align*}
...
パーセプトロンでは,活性化関数としてステップ関数が利用されており,その出力は必ず0か1でした. ニューラルネットワークでは,**シグモイド関数**や**ReLU関数(Rectified Linear Unit function)**などが利用され出力が0,1以外の値を取ります.
...
畳み込みニューラルネットワーク（Convolutional Neural Network, CNN）とは主に画像データの処理に適した特殊なニューラルネットワークの一種です. 通常のニューラルネットワークとは異なり, 入力データの空間的な構造（例: ピクセル間の位置関係）を活用して学習を行います.
