...
コンピュータのオペレーティングシステムとユーザー間のCLIを提供するプログラムをShellといい,Windowsでは,Command PromptやPowerShellなどがあります. MacなどのUnix系では,Bashやzshがあり,Terminalというソフトウェアを介して利用します.

本講義では環境や好みによって好きな環境で開発して構いませんが,ここでは,PowerShellの利用法を解説します.

Windows11の検索バーで `PowerShell`と検索して,出てきた `Windows PowerShell`をクリックしましょう.

![Screenshot PowerShell](/images/PowerShell-launch.png)

立ち上がった,青色の画面に文字でコマンド(命令)を入力して,コンピュータを操作します.

![Screenshot PowerShell](/images/powershell.png)
...
左上の`アイコン > フォント > フォントの部分を｢MSゴシック｣などの日本語フォントに変更しOKをクリックすることで,日本語が表示されるようになります.

![Screenshot PowerShell](/images/powershell-japanese.png)
...
- Windowsでは,ディレクトリを区切る文字が`¥`で表示されていると思います.
...
知らない部分,自分では書けない部分が見つかったらそれを理解するように勉強しましょう. ChatGPTに聞いても構いません. ただ,聞いたことを説明できるようにしましょう. 自分で説明できないものを使えるのは**せいぜい学校の課題くらいです**. 仕事や研究では,中身の良く分からないコードは利用できません. 最低限自分で理解したものだけを利用してください.
...
ある値が偶数かどうか判定するには,2で割ったあまりが0かどうかを判定すれば良い.
x=101,y=202として, 以下の命題の真偽を計算せよ.
