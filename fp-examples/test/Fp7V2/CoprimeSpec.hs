-- | fp7v2.md Exercise CH7-5 (互いに素 — 関係を内包表記で計算する) の回答例.
module Fp7V2.CoprimeSpec (spec) where
...
spec :: Spec
spec = describe "Fp7V2.Coprime (Exercise CH7-5)" $ do
  ...
  describe "coprimePairs (関係の有限版)" $ do
    it "coprimePairs 4 (fp7v2.md の期待値)" $
